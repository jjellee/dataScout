...
def _option_clause_negated(text):
    ...
    negations = ["행사할수없", "행사불가", "해당사항없음", "해당없음", "미해당",
                 "부여하지않", "부여되지않", "존재하지않"]
    if any(n in head for n in negations):
        return True
    # 정정공시 함정: '주) 단순정정 : 매도청구권에 관하여 실제 계약한 사실이 없습니다'처럼
    # 옵션 부인 문구가 (정정 전 날짜 서술 뒤인) 스니펫 뒤쪽에 오는 경우.
    # 아래 문구들은 옵션 섹션 안에서 나오면 부존재가 확실한 초특이적 표현이라 전체 검사해도 안전.
    t_all = text.replace(" ", "").replace("\n", "")
    strong_negations = ["계약한사실이없", "계약사실이없", "체결한사실이없", "체결된사실이없",
                        "부여된바없", "부여한바없", "약정한사실이없", "약정사실이없"]
    return any(s in t_all for s in strong_negations)
...
def extract_option_dates_llm(call_text, put_text):
    ...
    prompt = (
        ...
        "- 정정공시에는 '정정 전' 서술(날짜 포함)과 '주) 매도청구권에 관하여 실제 계약한 사실이 없습니다' 같은 "
        "정정/부인 문구가 함께 나올 수 있다. 부인 문구가 있으면 앞쪽에 날짜가 있어도 해당 옵션은 \"-\"다. "
        "뒤에 나오는 정정 문구가 앞의 서술보다 항상 우선한다.\n"
        "- put_start는 옵션을 처음 '행사(상환)'할 수 있는 날이다 (보통 '발행일로부터 1년이 되는 날인 X'). "
        "'지급일 전 60일부터 30일까지 청구' 같은 사전 접수기간이나 회차별 일정표의 FROM 날짜는 접수 창구일 뿐이므로 "
        "put_start로 쓰지 마라. 서술문의 행사가능일과 일정표가 함께 있으면 서술문의 행사가능일(첫 조기상환지급일)을 쓴다.\n"
